    ...
    try {
      ...
      if (pathname.startsWith('/api/')) {
        ...
        if (pathname === '/api/ai/analyze' && request.method === 'POST') {
          ...
          if (!prompt) return badRequest('Prompt obligatoire.');

          const result = await env.AI.run('@cf/meta/llama-3.1-8b-instruct', {
            prompt: `Tu es l'assistant stratégique de l'application État-Major Personnel.
Réponds en français, de façon claire, structurée et concise.
Ne prétends pas être une personne réelle.
